function rot13(str: string) {
  ...
  for (let i: number = 0; i < array.length; i++) {
    if (
      array[i].charCodeAt(0) < 65 ||
      array[i].charCodeAt(0) > 122 ||
      (array[i].charCodeAt(0) > 90 && array[i].charCodeAt(0) < 97)
    ) {
      continue;
    }
    if (array[i].charCodeAt(0) >= 65 && array[i].charCodeAt(0) <= 90) {
      array[i] = String.fromCharCode(
        65 + ((array[i].charCodeAt(0) + SHIFTER - 65) % ALPHABET)
      );
    } else if (array[i].charCodeAt(0) >= 97 && array[i].charCodeAt(0) <= 122) {
      array[i] = String.fromCharCode(
        97 + ((array[i].charCodeAt(0) + SHIFTER - 97) % ALPHABET)
      );
    }
  }
  return array.join("");
